...
class ImageFloder(data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            if(self.training and tn%2 == 0 and self.rand()>0.5):
            ...
            if self.crop_size is not None:
                ...
                if self.training: 
                    x1 = self.randint(0, w - tw) if w>tw else 0
                    y1 = self.randint(0, h - th) if h>th else 0
                else:
                    x1 = w - tw
                    y1 = h - th
                logger.debug('[x1, y1, x1+tw, y1+th] : ' + str([x1, y1, x1+tw, y1+th]))
            ...
            logger.error(msg)
            index = self.randint(0, len(self)-1)
